...
namespace a
{
    public enum MessageType { dinamicallyAddSlave, dinamicallyRemoveSlave, confirmMessageSuccess_Single, xml, uninitialized, masterChange, confirmMessageSuccess_Batch, provideSlaveList, argumentChange };
    public class myMessage : IComparable
    {
        ...
        //todo: devo fare un messageindex per ordinamento qquando stesso timestamp. e buttare al cesso i timestamp per sempre.
        public myMessage() { }
        ...
        public static string e { get { throw ex; } set { throw ex; } }
        public static string ret { get { throw ex; } set { throw ex; } }
        public static string tryMakeKey(string raw){
            string e = null;
            ...
            if ((equipStart = raw.IndexOf(equipS, 0)) == -1) { return e; }
            else {
                equipStart += equipS.Length;
                if (raw[equipStart] == '>'){
                    equipStart++;
                    if ((equipEnd = raw.IndexOf(equipE, equipStart)) == -1) { Program.pe("unterminated Equip_OID found in: " + raw); return e; }
            ...
            if ((recipeStart = raw.IndexOf(recipeS, equipEnd)) == -1) { return e; }
            else {
                recipeStart += recipeS.Length;
                if (raw[recipeStart] == '>'){
                    recipeStart++;
                    if ((recipeEnd = raw.IndexOf(recipeE, recipeStart)) == -1) { Program.pe("unterminated recipe_OID found in: " + raw); return e; }
                    recipe = raw.Substring(recipeStart, recipeEnd - recipeStart); }
                else if (raw[recipeStart] == '/') { recipe = ""; recipeEnd = recipeStart; }
                else {Program.pe("MakeKey error"); return raw; } }

            if ((stepStart = raw.IndexOf(stepS, recipeEnd)) == -1) { return e; }
            else {
                stepStart += stepS.Length;
                if (raw[stepStart] == '>'){
                    stepStart++;
                    if ((stepEnd = raw.IndexOf(stepE, stepStart)) == -1) { Program.pe("unterminated step_OID found in: " + raw); return e; }
                    step = raw.Substring(stepStart, stepEnd - stepStart); }
                else if (raw[stepStart] == '/') { step = ""; stepEnd = stepStart; }
                else {Program.pe("MakeKey error"); return raw; } }

            if ((holdtStart = raw.IndexOf(holdtS, stepEnd)) == -1) { return e; }
            else {
                holdtStart += holdtS.Length;
                if (raw[holdtStart] == '>'){
                    holdtStart++;
                    if ((holdtEnd = raw.IndexOf(holdtE, holdtStart)) == -1) { Program.pe("unterminated holdtype found in: " + raw); return e; }
                    holdt = raw.Substring(holdtStart, holdtEnd - holdtStart); }
                else if (raw[holdtStart] == '/') { holdt = ""; holdtEnd = holdtStart; }
                else {Program.pe("MakeKey error"); return raw; } }

            if ((holdfStart = raw.IndexOf(holdfS, holdtEnd)) == -1) { return e; }
            else {
                holdfStart += holdfS.Length;
                if (raw[holdfStart] == '>'){
                    holdfStart++;
                    if ((holdfEnd = raw.IndexOf(holdfE, holdfStart)) == -1) { Program.pe("unterminated holdflag found in: " + raw); return e; }
                    holdf = raw.Substring(holdfStart, holdfEnd - holdfStart); }
                else if (raw[holdfStart] == '/') { holdf = ""; holdfEnd = holdfStart; }
                else {Program.pe("MakeKey error"); return raw; } }

            if ((timeStart = raw.IndexOf(timeS, holdfEnd)) == -1) { return e; }
            else {
                timeStart += timeS.Length;
                if (raw[timeStart] == '>'){
                    //string fragment = raw.Substring(timeStart);
                    timeStart++;
                    if ((timeEnd = raw.IndexOf(timeE, timeStart)) == -1) { Program.pe("unterminated timestamp found in: " + raw); return e; }
        ...
        public void consume() {
            ...
            switch (type) {
                case MessageType.argumentChange:
                    ...
                    //todo: fai questo e crea anche un software che generi messaggi di ripartizionamento per gestire dinamicamente tutte le partizioni, un supermaster
                    break;
                case MessageType.masterChange:
